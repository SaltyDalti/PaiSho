...
namespace PaiSho.Game
{
    ...
    public class SeasonManager : MonoBehaviour
    {
        ...
        public bool SeasonJustRotated { get; private set; }

        public void AdvanceTurn()
        {
            SeasonJustRotated = false;
            turnCounter++;
            if (turnCounter >= turnsPerSeason)
            {
                RotateSeason();
                turnCounter = 0;
                SeasonJustRotated = true;
            }
        }
        ...
        public void ClearSeasonJustRotated() => SeasonJustRotated = false;

        /// <summary>Short player-facing hint for the season-change banner.</summary>
        public string DescribeSeasonHint(Season season)
        {
            return season switch
            {
                Season.Spring => "place a tile or chain 3+ harmonies for bonus points.",
                Season.Summer => "Freeze or Revive wilted tiles for a Momentum boost.",
                Season.Autumn => "Revive wilted tiles while keeping harmonies fresh.",
                Season.Winter => "move exactly one tile into harmony for the bonus.",
                _ => "watch the board for new seasonal bonuses."
            };
        }
        ...
        public bool EvaluateSeasonalBonuses(Player player, List<Piece> pieces)
        {
        ...
        if (scoreBonus > 0 || momentumBonus > 0)
        {
            if (BoardManager.Instance != null)
            {
                foreach (Piece piece in BoardManager.Instance.GetAllPieces())
                {
                    if (piece == null || piece.Owner != player)
                        continue;
                    if (!IsInSeason(piece.Type))
                        continue;
                    PieceStateAnimator.Ensure(piece)?.NotifySeasonalBoost();
                }
            }
